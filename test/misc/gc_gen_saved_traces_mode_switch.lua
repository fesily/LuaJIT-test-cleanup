...
local function record_saved_traces()
  for _ = 1, 4 do
    assert(make_root_chain(6) == 0)
  end
...
for i = 1, #ids do
  assert(not jutil.traceinfo(ids[i]), ids[i])
end

ids = record_saved_traces()
churn(80)
assert_traces_alive(ids)
for _ = 1, 3 do
  run_traces()
